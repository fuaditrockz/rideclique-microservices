...
function getCountAndMatch(args) {
  ...
  if (test === 'UNDEFINEDUNDEFINED') return [count, matchPattern];else if (test === 'MATCHUNDEFINED') matchPattern = pattern(args[1]);else if (test === 'COUNTUNDEFINED') count = parseInt(args[1], 10);else if (test === 'MATCHCOUNT') {
    matchPattern = pattern(args[1]);
    count = parseInt(args[3], 10);
  } else if (test.startsWith('MATCH') || test.startsWith('COUNT')) throw new Error('BAD Syntax');else throw new Error('Uknown option ' + args[0]);
...
function scanHelper(allKeys, size, cursorStart) {
  ...
  if (matchPattern) {
    var i = 0;
    while (i < keys.length) {
      if (!matchPattern(keys[i])) keys.splice(i, size);else i += size;
    }
  }
